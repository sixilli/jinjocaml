...
and is_jinja_start lexer =
  ...
  match p with
  | Some ch when Char.(ch = '{') -> advance @@ advance lexer, Token.Expression
  | Some ch when Char.(ch = '%') -> advance @@ advance lexer, Token.Statement
  | Some _ -> lexer, Token.LeftBracket
  | None -> lexer, Token.LeftBracket
